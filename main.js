class MatchingGame{
    ...
    generateBoard(){
        ...
        for(let i = 0; i < this.arr_of_images.length; i++){
            ...
            <img src="/img/question.png" id="${this.arr_of_images[i].id}" alt="question" data-card>
            `;
        }
    }
            ...
            if(!this.currentFlips[0]){
                ...
                for(let i = 0; i < this.arr_of_images.length - 1; i++){
                    if(this.arr_of_images[i].id == id){
                        singleCard.path[0].src = `/img/${this.arr_of_images[i].src}`;
                        break;
                    }
                }
                ...
                setTimeout(() => {
                    ...
                    singleCard.path[0].src = `/img/question.png`;
                    return;
                }, 400);
                this.currentFlips[0] = null;
                this.currentFlips[1] = null;
                setTimeout(() => {
                    this.wrongMatch[0].path[0].src = `/img/question.png`;
                    this.wrongMatch.pop();
                }, 400);
            }
            return;
...
const cardFlipAudio = new Audio('/mp3/card-flip.mp3');
const cardMatchAudio = new Audio('/mp3/bell_sound.mp3');
const errorNoiseAudio = new Audio('/mp3/error-noise.mp3');
const gameAudio = new Audio('/mp3/game_audio.mp3');
